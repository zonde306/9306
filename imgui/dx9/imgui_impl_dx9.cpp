...
WNDPROC g_pOldWindowProccess;
bool g_bIsKeyPressing[256];
HWND g_hwGameWindow;
bool g_bIsShowMenu = false;
FnMenuToggle g_pCallbackOnMenuToggle = nullptr;
...
LRESULT WINAPI ImGui_WindowProccess(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_LBUTTONDOWN:
		g_bIsKeyPressing[VK_LBUTTON] = true;
		break;
	case WM_LBUTTONUP:
		g_bIsKeyPressing[VK_LBUTTON] = false;
		break;
	case WM_RBUTTONDOWN:
		g_bIsKeyPressing[VK_RBUTTON] = true;
		break;
	case WM_RBUTTONUP:
		g_bIsKeyPressing[VK_RBUTTON] = false;
		break;
	case WM_MBUTTONDOWN:
		g_bIsKeyPressing[VK_MBUTTON] = true;
		break;
	case WM_MBUTTONUP:
		g_bIsKeyPressing[VK_MBUTTON] = false;
		break;
	case WM_XBUTTONDOWN:
		if(HIWORD(wParam) == XBUTTON1)
			g_bIsKeyPressing[VK_XBUTTON1] = true;
		else if(HIWORD(wParam) == XBUTTON2)
			g_bIsKeyPressing[VK_XBUTTON2] = true;
		break;
	case WM_XBUTTONUP:
		if (HIWORD(wParam) == XBUTTON1)
			g_bIsKeyPressing[VK_XBUTTON1] = false;
		else if (HIWORD(wParam) == XBUTTON2)
			g_bIsKeyPressing[VK_XBUTTON2] = false;
		break;
	case WM_KEYDOWN:
		g_bIsKeyPressing[wParam] = true;
		break;
	case WM_KEYUP:
		g_bIsKeyPressing[wParam] = false;
		break;
	}

	static bool isPressed = false;
	if (g_bIsKeyPressing[VK_INSERT])
		isPressed = true;
	else if (isPressed)
	{
		isPressed = false;
