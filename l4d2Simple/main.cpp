...
bool g_bKeyPressing[255];												// 某个键是否按下了
...
DWORD WINAPI StartCheat(LPVOID params)
{
	...
	if((PaintStartDrawing = (FnStartDrawing)Utils::FindPattern("vguimatsurface.dll", 
		...
		Utils::log("CMatSystemSurface::StartDrawing found client.dll + 0x%X", (DWORD)PaintStartDrawing - g_iMaterialModules);
	else
	...
	if ((PaintFinishDrawing = (FnFinishDrawing)Utils::FindPattern("vguimatsurface.dll",
		...
		Utils::log("CMatSystemSurface::FinishDrawing found client.dll + 0x%X", (DWORD)PaintFinishDrawing - g_iMaterialModules);
	else
...
void __fastcall Hooked_PaintTraverse(void* pPanel, void* edx, unsigned int panel, bool forcePaint, bool allowForce)
{
	...
	*/
	...
	// ((FnPaintTraverse)g_interface.PanelHook->GetOriginalFunction(indexes::PaintTraverse))(ecx, panel, forcePaint, allowForce);
...
bool __fastcall Hooked_EngineKeyEvent(CEngineVGui *ecx, void *edx, const InputEvent_t &event)
{
	...
	if (code > KEY_NONE)
		g_bIsKeyPressing[code] = isDown;

	return oEngineKeyEvent(ecx, event);
}
