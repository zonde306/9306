...
public:
	...
	int32_t GetIndex(const Iterator_t &it) const
	{
		Assert(IsValidIterator(it));
		if (!IsValidIterator(it))
			return InvalidIndex();

		return (int32_t)(HeaderToBlock(it.m_pBlockHeader) + it.m_nIndex);
	}
	bool IsIdxAfter(int32_t i, const Iterator_t &it) const
	{
		Assert(IsValidIterator(it));
		if (!IsValidIterator(it))
			return false;
	...
	T& Element(int32_t i);
	const T& Element(int32_t i) const;

	// Can we use this index?
	bool IsIdxValid(int32_t i) const;

	// Specify the invalid ('null') index that we'll only return on failure
	static const int32_t INVALID_INDEX = 0; // For use with COMPILE_TIME_ASSERT
	static int32_t InvalidIndex() { return INVALID_INDEX; }
...
protected:
	...
	bool IsInBlock(int32_t i, BlockHeader_t *pBlockHeader) const
	{
		T *p = (T*)i;
...
//-----------------------------------------------------------------------------
template< class T >
inline T& CUtlFixedMemory<T>::operator[](int32_t i)
{
	Assert(IsIdxValid(i));
	return *(T*)i;
}
...
template< class T >
inline const T& CUtlFixedMemory<T>::operator[](int32_t i) const
{
	Assert(IsIdxValid(i));
	return *(T*)i;
}
...
template< class T >
inline T& CUtlFixedMemory<T>::Element(int32_t i)
{
	Assert(IsIdxValid(i));
	return *(T*)i;
}
...
template< class T >
inline const T& CUtlFixedMemory<T>::Element(int32_t i) const
{
	Assert(IsIdxValid(i));
	return *(T*)i;
}
...
//-----------------------------------------------------------------------------
template< class T >
inline bool CUtlFixedMemory<T>::IsIdxValid(int32_t i) const
{
#ifdef _DEBUG
